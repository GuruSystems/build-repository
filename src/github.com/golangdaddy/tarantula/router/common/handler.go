...
func (handler *Handler) Handle(req web.RequestInterface) {
	...
	if handler.IsFile {
		...
		Respond(req, req.Respond(handler.fileCache))
		return
	}

	if handler.function == nil { req.Log().Panic("FAILED TO GET FUNCTION TO SERVE HANDLE OPERATION!") }

	Respond(req, handler.function(req))
	return
}
