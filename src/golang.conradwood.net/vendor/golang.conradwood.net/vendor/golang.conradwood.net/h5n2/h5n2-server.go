...
import (
	...
	pb "golang.conradwood.net/h5n2/proto"
...
func st(server *grpc.Server) error {
	...
	pb.RegisterH5N2ServiceServer(server, s)
	return nil
}
...
func (s *H5N2Server) Put(ctx context.Context, pr *pb.PutRequest) (*pb.PutResponse, error) {
...
func (s *H5N2Server) Get(ctx context.Context, pr *pb.GetRequest) (*pb.GetResponse, error) {
	peer, ok := peer.FromContext(ctx)
	if !ok {
		fmt.Println("Error getting peer ")
	}
